...
object ProductForm {
    ...
    fun setupFormForChanger(product: Product) {
        mainScope.launch {
            handleDropdownsForChanger(product)
            handleFormSubmit(true,id = product.id)
        }
    }
    fun setupForm() {
    ...
    suspend fun handleDropdownsForChanger(product: Product) {
        ...
        categoryDropdown?.value = product.category.id.toString()
        var nameRow = document.getElementById("description") as HTMLInputElement
        nameRow.value = product.name
        var priceRow = document.getElementById("price") as HTMLInputElement
        priceRow.value = product.price.toString()
    ...
    private fun handleFormSubmit(forupDate: Boolean,id: String = UUID().toString()) {
        val submitButton = document.getElementById(PRODUCTS_FORM_ID + "SubmitBtn") as? HTMLButtonElement
        submitButton?.onclick = {
            mainScope.launch {
                submitProduct(forupDate, id = id)
            }
        }
    }
    ...
    private suspend fun populateSubCategories(category: String) {
     ...
     private suspend fun submitProduct(forupDate: Boolean, id: String) {
        ...
        if (description.isNotBlank() && price != null) { // add more checks here...
            ...
            val product = Product(id, description, Category(categoryId.toLong(), ""), price)
            if (forupDate){
